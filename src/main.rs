...
impl Debug for DivOutput {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.write_str(
            &format!("Number: {}, Remainder: {}", self.num, self.remainder)
        )
    }
}
...
fn division_with_remainder(number: f64, division: f64) -> DivOutput {
    ...
    DivOutput { num: whole_num, remainder: remainder.round() }
}
...
impl DecimalPair {
    fn new(number: Decimal, base: i64, nth: usize) -> Self {
        Self { number, base: BasePair::new(base, nth) }
    }
    ...
    fn into_vec_pairs(number: i64, base: i64) -> Vec<DecimalPair> {
        ...
        for (index, item) in number.to_string().chars().rev().enumerate() {
            ...
            vec_pairs.push(
                dbg!(DecimalPair::new(Decimal::new(item_num), base, index))
            )
        }
...
impl Debug for DecimalPair {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.write_str(
            &format!("Num: {:?}, Base: {:?}", self.number, self.base)
        )
    }
}
...
fn main() -> anyhow::Result<()> {
    ...
    let decimal_vec = DecimalPair::into_vec_pairs(number.trim().parse()?, szamrendszer.trim().parse()?);
